...
def load_data(file_path):
    ...
    input_data = df.iloc[:, :46].values.astype('float32')
    output_data = df.iloc[:, 46].values.astype('float32')
...
CSV_FILE_PATH = "D:/pythonproject/Cefixime_feature_abstraction/Cefixime_data/Cefixime/cefixime_Oaddition_alldata.csv"
...
history = model.fit(X_train, y_train, epochs=10000, batch_size=2, validation_split=0.1, callbacks=[model_checkpoint])
...
df1.to_csv('./loss_data2.csv', index=False)
